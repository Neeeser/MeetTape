...
///
/// Skipped unless `MEETTAPE_LIVE_OPENAI=1` and `OPENAI_API_KEY` are both set, so
...
enum LiveOpenAITests {
    static var isEnabled: Bool {
        ProcessInfo.processInfo.environment["MEETTAPE_LIVE_OPENAI"] == "1"
    }

    static var fixtureDirectory: URL? {
        if let path = ProcessInfo.processInfo.environment["MEETTAPE_LIVE_FIXTURE"] {
    ...
    static func requireLive() throws -> (client: OpenAIClient, fixtures: URL) {
        guard isEnabled else {
            throw TestSkip("set MEETTAPE_LIVE_OPENAI=1 to run live API tests")
        }
