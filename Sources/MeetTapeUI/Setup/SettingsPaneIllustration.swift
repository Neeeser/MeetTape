...
/// row shows MeetTape's real icon, follows light and dark, and cannot go stale.
///
/// It shows the state being asked for, not the state that exists. Someone who
/// has just removed MeetTape from the real list sees a picture that still has it,
...
/// MeetTape's own icon, at whatever size the caller draws it.
...
public enum ApplicationIdentity {
    /// What MeetTape is called in prose.
    static var name: String {
        Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "MeetTape"
    }
    ...
    /// built from that reads "MeetTape" against a real row saying "MeetTape.app".
    static var settingsListName: String {
        let file = Bundle.main.bundleURL.lastPathComponent
        return file.isEmpty ? "\(name).app" : file
    }

    /// The dragged item: MeetTape's application bundle, as a file URL.
    ///
...
extension View {
    /// Makes this view drag MeetTape's application bundle.
    ///
...
/// MeetTape's icon, draggable into a System Settings list.
///
